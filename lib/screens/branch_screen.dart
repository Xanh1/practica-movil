import 'package:flutter/material.dart';
import 'package:prueba_1/controllers/services.dart';
import 'package:prueba_1/models/data_response.dart';

class Branchcreen extends StatefulWidget {
  const Branchcreen({Key? key}) : super(key: key);

  @override
  _branchescreenState createState() => _branchescreenState();
}

class _branchescreenState extends State<Branchcreen> {
  List<Map<String, dynamic>> _branches = [];
  bool _isLoading = true;
  int _selectedIndex = 0;

  @override
  void initState() {
    super.initState();
    _loadBranch();
  }

  Future<void> _loadBranch() async {
    final Services services = Services();
    final DataResponse response = await services.getBranches();

    if (response.code == "200") {
      setState(() {
        _branches = List<Map<String, dynamic>>.from(response.context);
        _isLoading = false;
      });
    } else {
      setState(() {
        _isLoading = false;
      });
    }
  }

  void _onItemTapped(int index) {
    setState(() {
      _selectedIndex = index;

      switch (index) {
        case 0:
          Navigator.pushNamed(context, '/dashboard/user');
          break;
        case 1:
          Navigator.pushNamed(context, '/dashboard/map');
          break;
        case 2:
          Navigator.pushNamed(context, '/dashboard/branch');
          break;
        case 3:
          Navigator.pushNamed(context, '/dashboard/products');
          break;
      }
    });
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Branches"),
      ),
      body: Center(
        child: _isLoading
            ? const CircularProgressIndicator()
            : BranchTable(data: _branches),
      ),
      bottomNavigationBar: BottomNavigationBar(
        items: const <BottomNavigationBarItem>[
          BottomNavigationBarItem(
            icon: Icon(Icons.person),
            label: 'Profile',
          ),
          BottomNavigationBarItem(
            icon: Icon(Icons.map),
            label: 'Map',
          ),
          BottomNavigationBarItem(icon: Icon(Icons.store), label: 'Branch'),
          BottomNavigationBarItem(icon: Icon(Icons.list_alt), label: 'Product'),
        ],
        currentIndex: _selectedIndex,
        selectedItemColor: Colors.black,
        unselectedItemColor: Colors.black,
        onTap: _onItemTapped, // Conectar el método con el evento onTap
      ),
    );
  }
}

class BranchTable extends StatelessWidget {
  final List<Map<String, dynamic>> data;

  const BranchTable({super.key, required this.data});

  @override
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      child: DataTable(
        showCheckboxColumn: false,
        columns: const [
          DataColumn(label: Text('Name')),
          DataColumn(label: Text('LOngitude')),
          DataColumn(label: Text('Latitude')),
        ],
        rows: data.map((item) {
          return DataRow(
            cells: [
              DataCell(Text(item['name'] ?? '')),
              DataCell(Text(item['longitude'].toString())),
              DataCell(Text(item['latitude'].toString())),
            ],
          );
        }).toList(),
      ),
    );
  }
}
